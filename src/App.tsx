...
import { exportElementToPdf, sanitizeFileNameSegment } from "./lib/pdfExport";
import type {
  JobInfo,
  OssConfig,
  OssProviderKind,
...
function App() {
  ...
  async function onExportSummaryPdf() {
    try {
      ...
      const fileName = await exportElementToPdf({
        rootSelector: ".summary-print-root",
        fileName: `${sessionLabel || "summary"}-summary`,
        missingRootError: "summary view not found"
      });
  ...
  async function onExportDiscoverPdf(fileName: string) {
    try {
      const savedFileName = await exportElementToPdf({
        rootSelector: ".discover-print-root",
        fileName,
        missingRootError: t("discover.report.notReady"),
        footerText: t("discover.report.footer")
      });
      setStatus("status.discoverPdfFinished", { fileName: savedFileName });
    } catch (error) {
      setStatus("status.discoverPdfFailed", { error: String(error) });
      console.error("[discover-pdf] failed to export discover pdf", { error: String(error) });
    }
  }
  ...
  return (
    <main className="app-shell">
      ...
      >
        <DiscoverTab
          sessions={sessions}
          onExportPdf={(fileName) => void onExportDiscoverPdf(fileName)}
          t={t}
        />
      </div>
